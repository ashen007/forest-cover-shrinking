...
from forest_cover_change_detection.models.fcef.modules import ResidualFCA, UpSample


class FCFEResFCA(nn.Module):

    def __init__(self, in_channels, classes, kernel=3):
        super(FCFEResFCA, self).__init__()
        filters = [16, 32, 64, 128, 256]
        self.drop = nn.Dropout(0.2)

        # down sampling
        self.feat_ext_block_1 = nn.Sequential(ResidualFCA(in_channels, filters[0]),
                                              ResidualFCA(filters[0], filters[0]), )
        self.dwn_block_1 = nn.MaxPool2d(2)  # (16, 128, 128)

        self.feat_ext_block_2 = nn.Sequential(ResidualFCA(filters[0], filters[1]),
                                              ResidualFCA(filters[1], filters[1]), )
        self.dwn_block_2 = nn.MaxPool2d(2)  # (32, 64, 64)

        self.feat_ext_block_3 = nn.Sequential(ResidualFCA(filters[1], filters[2]),
                                              ResidualFCA(filters[2], filters[2]), )
        self.dwn_block_3 = nn.MaxPool2d(2)  # (64, 32, 32)

        self.feat_ext_block_4 = nn.Sequential(ResidualFCA(filters[2], filters[3]),
                                              ResidualFCA(filters[3], filters[3]), )
        self.dwn_block_4 = nn.MaxPool2d(2)  # (128, 16, 16)

        self.feat_ext_block_5 = nn.Sequential(ResidualFCA(filters[3], filters[4]),
                                              ResidualFCA(filters[4], filters[4]), )
        self.dwn_block_5 = nn.MaxPool2d(2)  # (256, 8, 8)

        # up sampling
        self.up_feat_ext_block_1 = nn.Sequential(ResidualFCA(filters[4], filters[4]),
                                                 ResidualFCA(filters[4], filters[4]))
        ...
        self.up_feat_ext_block_2 = nn.Sequential(ResidualFCA(3 * filters[3], filters[4]),
                                                 ResidualFCA(filters[4], filters[4]))
        # this block is the layer that increases the dimensions by factor 2
        ...
        # this is a common up-sample block for all models
        self.up_feat_ext_block_3 = nn.Sequential(ResidualFCA(3 * filters[2], filters[3]),
                                                 ResidualFCA(filters[3], filters[3]))
        # this block is the layer that increases the dimensions by factor 2
        ...
        # this is a common up-sample block for all models
        self.up_feat_ext_block_4 = nn.Sequential(ResidualFCA(3 * filters[1], filters[2]),
                                                 ResidualFCA(filters[2], filters[2]))
        # this block is the layer that increases the dimensions by factor 2
        ...
        # this is a common up-sample block for all models
        self.up_feat_ext_block_5 = nn.Sequential(ResidualFCA(3 * filters[0], filters[1]),
                                                 ResidualFCA(filters[1], filters[1]))
        # this block is the layer that increases the dimensions by factor 2

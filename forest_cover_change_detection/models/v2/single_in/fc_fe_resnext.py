...
class FCFEResNeXt(nn.Module):
    ...
    def __init__(self, in_channels, classes, kernel=3):
        ...
        # down sampling
        self.feat_ext_block_1 = nn.Sequential(ResNeXtDownSample(in_channels, filters[0]),
                                              ResNeXtDownSample(filters[0], filters[0]), )
        self.dwn_block_1 = nn.MaxPool2d(2)  # (16, 128, 128)

        self.feat_ext_block_2 = nn.Sequential(ResNeXtDownSample(filters[0], filters[1]),
                                              ResNeXtDownSample(filters[1], filters[1]), )
        self.dwn_block_2 = nn.MaxPool2d(2)  # (32, 64, 64)

        self.feat_ext_block_3 = nn.Sequential(ResNeXtDownSample(filters[1], filters[2]),
                                              ResNeXtDownSample(filters[2], filters[2]), )
        self.dwn_block_3 = nn.MaxPool2d(2)  # (64, 32, 32)

        self.feat_ext_block_4 = nn.Sequential(ResNeXtDownSample(filters[2], filters[3]),
                                              ResNeXtDownSample(filters[3], filters[3]), )
        self.dwn_block_4 = nn.MaxPool2d(2)  # (128, 16, 16)

        self.feat_ext_block_5 = nn.Sequential(ResNeXtDownSample(filters[3], filters[4]),

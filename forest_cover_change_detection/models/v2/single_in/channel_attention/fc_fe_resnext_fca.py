...
class FCFEResNeXtFCA(nn.Module):
    ...
    def __init__(self, in_channels, classes, kernel=3):
        ...
        self.feat_ext_block_1 = nn.Sequential(nn.Conv2d(in_channels, filters[0], 3, padding=1),
                                              ResNeXtFCA(filters[0], filters[0], c),
                                              ResNeXtFCA(filters[0], filters[0], c),
                                              )
        ...
        # up sampling
        self.up_feat_ext_block_1 = nn.Sequential(ResNeXtFCA(filters[4], filters[4], c),
                                                 ResNeXtFCA(filters[4], filters[4], c),
                                                 )
        ...
        self.up_feat_ext_block_2 = nn.Sequential(ResNeXtFCA(3 * filters[3], filters[3], c),
                                                 ResNeXtFCA(filters[3], filters[4], c),
                                                 )
        # this block is the layer that increases the dimensions by factor 2
        ...
        # this is a common up-sample block for all models
        self.up_feat_ext_block_3 = nn.Sequential(ResNeXtFCA(3 * filters[2], filters[2], c),
                                                 ResNeXtFCA(filters[2], filters[3], c),
                                                 )
        # this block is the layer that increases the dimensions by factor 2
        ...
        # this is a common up-sample block for all models
        self.up_feat_ext_block_4 = nn.Sequential(ResNeXtFCA(3 * filters[1], filters[1], c),
                                                 ResNeXtFCA(filters[1], filters[2], c),
                                                 )
        # this block is the layer that increases the dimensions by factor 2
        ...
        # this is a common up-sample block for all models
        self.up_feat_ext_block_5 = nn.Sequential(ResNeXtFCA(3 * filters[0], filters[0], c),
                                                 ResNeXtFCA(filters[0], filters[1], c),
                                                 )
        # this block is the layer that increases the dimensions by factor 2
    ...
    def forward(self, x):
        ...
        xd_1 = self.dwn_block_1(xd_1_fet_ext)  # (16, 16, 48, 48)
        s_1 = xd_1.shape

        xd_2_fet_ext = self.feat_ext_block_2(xd_1)
        xd_2 = self.dwn_block_2(xd_2_fet_ext)  # (16, 32, 24, 24)
        s_2 = xd_2.shape

        xd_3_fet_ext = self.feat_ext_block_3(xd_2)
        xd_3 = self.dwn_block_3(xd_3_fet_ext)  # (16, 64, 12, 12)
        s_3 = xd_3.shape

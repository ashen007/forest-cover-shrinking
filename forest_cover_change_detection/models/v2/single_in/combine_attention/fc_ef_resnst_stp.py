...
from forest_cover_change_detection.models.fcef.modules import ResidualDownSample, UpSample, ResNeStBlock
from forest_cover_change_detection.models.fcfe_with_att.modules import StripPooling


class FCFEResSplitAttentionSP(nn.Module):

    def __init__(self, in_channels, classes, kernel=3):
        super(FCFEResSplitAttentionSP, self).__init__()
    ...
    def forward(self, x):
        ...
        xd_1 = self.dwn_block_1(xd_1_fet_ext)  # (16, 16, 128, 128)
        s_1 = xd_1.shape

        xd_2_fet_ext = self.feat_ext_block_2(xd_1)
        xd_2 = self.dwn_block_2(xd_2_fet_ext)  # (16, 32, 64, 64)
        s_2 = xd_2.shape

        xd_3_fet_ext = self.feat_ext_block_3(xd_2)
        xd_3 = self.dwn_block_3(xd_3_fet_ext)  # (16, 64, 32, 32)
        s_3 = xd_3.shape

        xd_4_fet_ext = self.feat_ext_block_4(xd_3)
        xd_4 = self.dwn_block_4(xd_4_fet_ext)  # (16, 128, 16, 16)
        s_4 = xd_4.shape

        xd_5_fet_ext = self.feat_ext_block_5(xd_4)
        xd_5 = self.dwn_block_5(xd_5_fet_ext)  # (16, 256, 8, 8)
        s_5 = xd_5.shape
        ...
        sp_1 = StripPooling(xd_4.shape[1], (8, 8))

        # concat skip connection and up-sampled layer from below
        xu_1 = torch.cat((sp_1(xd_4), pad_1(xu_1)), dim=1)
        ...
        sp_2 = StripPooling(xd_3.shape[1], (16, 16))

        # concat skip connection and up-sampled layer from below
        xu_2 = torch.cat((sp_2(xd_3), pad_2(xu_2)), dim=1)
        ...
        sp_3 = StripPooling(xd_2.shape[1], (32, 32))

        # concat skip connection and up-sampled layer from below
        xu_3 = torch.cat((sp_3(xd_2), pad_3(xu_3)), dim=1)
        ...
        sp_4 = StripPooling(xd_1.shape[1], (64, 64))

        # concat skip connection and up-sampled layer from below
        xu_4 = torch.cat((sp_4(xd_1), pad_4(xu_4)), dim=1)
...
if __name__ == '__main__':
    ...
    model = FCFEResSplitAttentionSP(6, 2)
    model.cuda()

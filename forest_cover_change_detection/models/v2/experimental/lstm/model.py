...
from forest_cover_change_detection.models.v2.experimental.lstm.modules import SetValues
...
class MultiPathV1(nn.Module):
    ...
    def __init__(self, in_channels, classes, kernel=3):
        ...
        # self.up_block_lstm_1 = UpSample(filters[4], filters[3], kernel,
        #                                 stride=2, padding=0, output_padding=1, blocks=1)  # (128, 16, 16)
        ...
        # self.up_block_lstm_2 = UpSample(filters[3], filters[2], kernel,
        #                                 stride=2, padding=0, output_padding=1, blocks=1)  # (64, 32, 32)
        ...
        # self.up_block_lstm_3 = UpSample(filters[2], filters[1], kernel,
        #                                 stride=2, padding=0, output_padding=1, blocks=1)  # (32, 64, 64)
        ...
        # self.up_block_lstm_4 = UpSample(filters[1], filters[0], kernel,
        #                                 stride=2, padding=0, output_padding=1, blocks=1)  # (32, 128, 128)
        ...
        # self.up_block_lstm_5 = UpSample(filters[0], filters[0], kernel,
        #                                 stride=2, padding=0, output_padding=1, blocks=1)  # (16, 256, 256)

        self.out_block = nn.Sequential(nn.Conv2d(2 * filters[0], classes, 1),
    ...
    def forward(self, x_1, x_2):
        ...
        # extracted features
        features_5 = (xd_5_b1 - xd_5_b2)
        features_4 = (xd_4_b1 - xd_4_b2)
        features_3 = (xd_3_b1 - xd_3_b2)
        features_2 = (xd_2_b1 - xd_2_b2)
        features_1 = (xd_1_b1 - xd_1_b2)

        lstm_5_after = self.lstm5(features_5)  # 256
        lstm_4_after = self.lstm4(features_4)  # 128
        lstm_3_after = self.lstm3(features_3)  # 64
        lstm_2_after = self.lstm2(features_2)  # 32
        lstm_1_after = self.lstm1(features_1)  # 16

        # conv decoder
        xu_1_b = self.up_feat_ext_block_1(features_5)
        ...
        # xu_1 = torch.cat((at_gate_1(features_4, xu_1_b), pad_1(xu_1)), dim=1)
        xu_1 = torch.cat((features_4, pad_1(xu_1)), dim=1)
        ...
        # concat skip connection and up-sampled layer from below
        #         xu_2 = torch.cat((at_gate_2(features_3, xu_2_b), pad_2(xu_2)), dim=1)
        xu_2 = torch.cat((features_3, pad_2(xu_2)), dim=1)
        ...
        # concat skip connection and up-sampled layer from below
        #         xu_3 = torch.cat((at_gate_3(features_2, xu_3_b), pad_3(xu_3)), dim=1)
        xu_3 = torch.cat((features_2, pad_3(xu_3)), dim=1)
        ...
        # concat skip connection and up-sampled layer from below
        #         xu_4 = torch.cat((at_gate_4(features_1, xu_4_b), pad_4(xu_4)), dim=1)
        xu_4 = torch.cat((features_1, pad_4(xu_4)), dim=1)
        ...
        # lstm decoder
        xu_1_lstm = self.up_feat_ext_block_1(lstm_5_after)
        xu_lstm_1 = self.up_block_1(xu_1_lstm)
        lstm_xu_1 = torch.cat((pad_1(xu_lstm_1), lstm_4_after), dim=1)

        xu_2_lstm = self.up_feat_ext_block_2(lstm_xu_1)
        xu_lstm_2 = self.up_block_2(xu_2_lstm)
        lstm_xu_2 = torch.cat((pad_1(xu_lstm_2), lstm_3_after), dim=1)

        xu_3_lstm = self.up_feat_ext_block_3(lstm_xu_2)
        xu_lstm_3 = self.up_block_3(xu_3_lstm)
        lstm_xu_3 = torch.cat((pad_1(xu_lstm_3), lstm_2_after), dim=1)

        xu_4_lstm = self.up_feat_ext_block_4(lstm_xu_3)
        xu_lstm_4 = self.up_block_4(xu_4_lstm)
        lstm_xu_4 = torch.cat((pad_1(xu_lstm_4), lstm_1_after), dim=1)

        xu_5_lstm = self.up_feat_ext_block_5(lstm_xu_4)
        xu_lstm_5 = self.up_block_5(xu_5_lstm)

        lstm_x_out = pad_5(xu_lstm_5)

        return self.out_block(torch.cat((x_out, lstm_x_out), dim=1))


if __name__ == '__main__':
    t_1 = torch.randn(16, 3, 128, 128).cuda()
    t_2 = torch.randn(16, 3, 128, 128).cuda()
    model = MultiPathV1(3, 2)
    model.cuda()

    print('output', model(t_1, t_2).shape)

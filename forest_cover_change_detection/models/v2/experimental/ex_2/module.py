import torch

from torch import nn
from torch.nn.modules.padding import ReplicationPad2d
from forest_cover_change_detection.models.fcef.modules import ResidualDownSample, UpSample
from forest_cover_change_detection.models.fcef.modules import ResNeStBlock
from forest_cover_change_detection.models.v2.experimental.new_gates.modules import DualAttentionV3, DualAttentionV4


class FCFEResSplitAttention(nn.Module):
    """
    improve an additive attention gate by adding channel and spatial
    attention blocks instead of the original block

    experiments:
        1. test both gates
        2. test addition vs. concatenation of paths
        3. capture the effect of strip pooling
    """

    def __init__(self, in_channels, classes=2, kernel=3, additive=True, type_='gate1', sp=False):
        super(FCFEResSplitAttention, self).__init__()
        filters = [16, 32, 64, 128, 256]
        self.additive = additive
        self.type_ = type_
        self.sp = sp

        # down sampling
        self.feat_ext_block_1 = nn.Sequential(ResidualDownSample(in_channels, filters[0]),
                                              ResNeStBlock(filters[0], filters[0]),
                                              ResNeStBlock(filters[0], filters[0]))
        self.dwn_block_1 = nn.MaxPool2d(2)  # (16, 128, 128)

        self.feat_ext_block_2 = nn.Sequential(ResidualDownSample(filters[0], filters[1]),
                                              ResNeStBlock(filters[1], filters[1]),
                                              ResNeStBlock(filters[1], filters[1]))
        self.dwn_block_2 = nn.MaxPool2d(2)  # (32, 64, 64)

        self.feat_ext_block_3 = nn.Sequential(ResidualDownSample(filters[1], filters[2]),
                                              ResNeStBlock(filters[2], filters[2]),
                                              ResNeStBlock(filters[2], filters[2]))
        self.dwn_block_3 = nn.MaxPool2d(2)  # (64, 32, 32)

        self.feat_ext_block_4 = nn.Sequential(ResidualDownSample(filters[2], filters[3]),
                                              ResNeStBlock(filters[3], filters[3]),
                                              ResNeStBlock(filters[3], filters[3]))
        self.dwn_block_4 = nn.MaxPool2d(2)  # (128, 16, 16)

        self.feat_ext_block_5 = nn.Sequential(ResidualDownSample(filters[3], filters[4]),
                                              ResNeStBlock(filters[4], filters[4]),
                                              ResNeStBlock(filters[4], filters[4]))
        self.dwn_block_5 = nn.MaxPool2d(2)  # (256, 8, 8)

        # up sampling
        self.up_feat_ext_block_1 = nn.Sequential(ResidualDownSample(filters[4], filters[4], kernel),
                                                 ResNeStBlock(filters[4], filters[4]),
                                                 ResNeStBlock(filters[4], filters[4]))
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_1 = UpSample(filters[4], filters[4], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (256, 16, 16)

        # this is a common up-sample block for all models
        if type_ == 'gate1':
            if not sp:
                layers_1 = [ResidualDownSample(3 * filters[3], filters[4], kernel),
                            ResNeStBlock(filters[4], filters[4]),
                            ResNeStBlock(filters[4], filters[4])
                            ]
            else:
                layers_1 = [ResidualDownSample(4 * filters[3], filters[4], kernel),
                            ResNeStBlock(filters[4], filters[4]),
                            ResNeStBlock(filters[4], filters[4])]
        else:
            if not sp:
                layers_1 = [ResidualDownSample(4 * filters[3], filters[4], kernel),
                            ResNeStBlock(filters[4], filters[4]),
                            ResNeStBlock(filters[4], filters[4])]
            else:
                layers_1 = [ResidualDownSample(5 * filters[3], filters[4], kernel),
                            ResNeStBlock(filters[4], filters[4]),
                            ResNeStBlock(filters[4], filters[4])]

        self.up_feat_ext_block_2 = nn.Sequential(*layers_1)
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_2 = UpSample(filters[4], filters[3], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (128, 32, 32)

        # this is a common up-sample block for all models
        if type_ == 'gate1':
            if not sp:
                layers_2 = [ResidualDownSample(3 * filters[2], filters[3], kernel),
                            ResNeStBlock(filters[3], filters[3]),
                            ResNeStBlock(filters[3], filters[3])
                            ]
            else:
                layers_2 = [ResidualDownSample(4 * filters[2], filters[3], kernel),
                            ResNeStBlock(filters[3], filters[3]),
                            ResNeStBlock(filters[3], filters[3])]
        else:
            if not sp:
                layers_2 = [ResidualDownSample(4 * filters[2], filters[3], kernel),
                            ResNeStBlock(filters[3], filters[3]),
                            ResNeStBlock(filters[3], filters[3])]
            else:
                layers_2 = [ResidualDownSample(5 * filters[2], filters[3], kernel),
                            ResNeStBlock(filters[3], filters[3]),
                            ResNeStBlock(filters[3], filters[3])]

        self.up_feat_ext_block_3 = nn.Sequential(*layers_2)
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_3 = UpSample(filters[3], filters[2], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (64, 64, 64)

        # this is a common up-sample block for all models
        if type_ == 'gate1':
            if not sp:
                layers_3 = [ResidualDownSample(3 * filters[1], filters[2], kernel),
                            ResNeStBlock(filters[2], filters[2]),
                            ResNeStBlock(filters[2], filters[2])
                            ]
            else:
                layers_3 = [ResidualDownSample(4 * filters[1], filters[2], kernel),
                            ResNeStBlock(filters[2], filters[2]),
                            ResNeStBlock(filters[2], filters[2])]
        else:
            if not sp:
                layers_3 = [ResidualDownSample(4 * filters[1], filters[2], kernel),
                            ResNeStBlock(filters[2], filters[2]),
                            ResNeStBlock(filters[2], filters[2])]
            else:
                layers_3 = [ResidualDownSample(5 * filters[1], filters[2], kernel),
                            ResNeStBlock(filters[2], filters[2]),
                            ResNeStBlock(filters[2], filters[2])]

        self.up_feat_ext_block_4 = nn.Sequential(*layers_3)
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_4 = UpSample(filters[2], filters[1], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (32, 128, 128)

        # this is a common up-sample block for all models
        if type_ == 'gate1':
            if not sp:
                layers_4 = [ResidualDownSample(3 * filters[0], filters[1], kernel),
                            ResNeStBlock(filters[1], filters[1]),
                            ResNeStBlock(filters[1], filters[1])
                            ]
            else:
                layers_4 = [ResidualDownSample(4 * filters[0], filters[1], kernel),
                            ResNeStBlock(filters[1], filters[1]),
                            ResNeStBlock(filters[1], filters[1])]
        else:
            if not sp:
                layers_4 = [ResidualDownSample(4 * filters[0], filters[1], kernel),
                            ResNeStBlock(filters[1], filters[1]),
                            ResNeStBlock(filters[1], filters[1])]
            else:
                layers_4 = [ResidualDownSample(5 * filters[0], filters[1], kernel),
                            ResNeStBlock(filters[1], filters[1]),
                            ResNeStBlock(filters[1], filters[1])]

        self.up_feat_ext_block_5 = nn.Sequential(*layers_4)
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_5 = UpSample(filters[1], filters[0], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (16, 256, 256)

        self.out_block = nn.Sequential(nn.Conv2d(filters[0], classes, kernel, padding=1),
                                       nn.LogSoftmax(dim=1))  # (2, 256, 256)

    def forward(self, x_1, x_2):
        s_0 = x_1.shape

        # branch 1
        xd_1_fet_ext_b1 = self.feat_ext_block_1(x_1)
        xd_1_b1 = self.dwn_block_1(xd_1_fet_ext_b1)  # 1(6, 16, 64, 64)
        s_1 = xd_1_b1.shape

        xd_2_fet_ext_b1 = self.feat_ext_block_2(xd_1_b1)
        xd_2_b1 = self.dwn_block_2(xd_2_fet_ext_b1)  # 1(6, 32, 32, 32)
        s_2 = xd_2_b1.shape

        xd_3_fet_ext_b1 = self.feat_ext_block_3(xd_2_b1)
        xd_3_b1 = self.dwn_block_3(xd_3_fet_ext_b1)  # 1(6, 64, 16, 16)
        s_3 = xd_3_b1.shape

        xd_4_fet_ext_b1 = self.feat_ext_block_4(xd_3_b1)
        xd_4_b1 = self.dwn_block_4(xd_4_fet_ext_b1)  # (16, 128, 8, 8)
        s_4 = xd_4_b1.shape

        xd_5_fet_ext_b1 = self.feat_ext_block_5(xd_4_b1)
        xd_5_b1 = self.dwn_block_5(xd_5_fet_ext_b1)  # (16, 256, 4, 4)
        s_5 = xd_5_b1.shape

        # branch 2
        xd_1_fet_ext_b2 = self.feat_ext_block_1(x_2)
        xd_1_b2 = self.dwn_block_1(xd_1_fet_ext_b2)  # (16, 16, 64, 64)

        xd_2_fet_ext_b2 = self.feat_ext_block_2(xd_1_b2)
        xd_2_b2 = self.dwn_block_2(xd_2_fet_ext_b2)  # (16, 32, 32, 32)

        xd_3_fet_ext_b2 = self.feat_ext_block_3(xd_2_b2)
        xd_3_b2 = self.dwn_block_3(xd_3_fet_ext_b2)  # (16, 64, 16, 16)

        xd_4_fet_ext_b2 = self.feat_ext_block_4(xd_3_b2)
        xd_4_b2 = self.dwn_block_4(xd_4_fet_ext_b2)  # (16, 128, 8, 8)

        xd_5_fet_ext_b2 = self.feat_ext_block_5(xd_4_b2)
        xd_5_b2 = self.dwn_block_5(xd_5_fet_ext_b2)  # (16, 256, 4, 4)

        # decoder
        xu_1_b = self.up_feat_ext_block_1((xd_5_b1 - xd_5_b2))
        xu_1 = self.up_block_1(xu_1_b)
        pad_1 = ReplicationPad2d((0, (s_4[3] - xu_1.shape[3]), 0, (s_4[2] - xu_1.shape[2])))

        if self.type_ == 'gate1':
            if self.sp:
                gate_1 = DualAttentionV3(256, s_4[1], self.additive, (2, 4))
            else:
                gate_1 = DualAttentionV3(256, s_4[1], self.additive)
        else:
            if self.sp:
                gate_1 = DualAttentionV4(256, s_4[1], self.additive, (2, 4))
            else:
                gate_1 = DualAttentionV4(256, s_4[1], self.additive)

        # concat skip connection and up-sampled layer from below
        skip_conn = (xd_4_b1 - xd_4_b2)
        attend_1 = gate_1(skip_conn, (xd_5_b1 - xd_5_b2))

        xu_1 = torch.cat((attend_1, pad_1(xu_1)), dim=1)

        xu_2_b = self.up_feat_ext_block_2(xu_1)
        xu_2 = self.up_block_2(xu_2_b)
        pad_2 = ReplicationPad2d((0, (s_3[3] - xu_2.shape[3]), 0, (s_3[2] - xu_2.shape[2])))

        if self.type_ == 'gate1':
            if self.sp:
                gate_2 = DualAttentionV3(256, s_3[1], self.additive, (4, 4))
            else:
                gate_2 = DualAttentionV3(256, s_3[1], self.additive)
        else:
            if self.sp:
                gate_2 = DualAttentionV4(256, s_3[1], self.additive, (4, 4))
            else:
                gate_2 = DualAttentionV4(256, s_3[1], self.additive)

        # concat skip connection and up-sampled layer from below
        skip_conn = (xd_3_b1 - xd_3_b2)
        attend_2 = gate_2(skip_conn, (xd_5_b1 - xd_5_b2))

        xu_2 = torch.cat((attend_2, pad_2(xu_2)), dim=1)

        xu_3_b = self.up_feat_ext_block_3(xu_2)
        xu_3 = self.up_block_3(xu_3_b)
        pad_3 = ReplicationPad2d((0, (s_2[3] - xu_3.shape[3]), 0, (s_2[2] - xu_3.shape[2])))

        if self.type_ == 'gate1':
            if self.sp:
                gate_3 = DualAttentionV3(256, s_2[1], self.additive, (4, 8))
            else:
                gate_3 = DualAttentionV3(256, s_2[1], self.additive)
        else:
            if self.sp:
                gate_3 = DualAttentionV4(256, s_2[1], self.additive, (4, 8))
            else:
                gate_3 = DualAttentionV4(256, s_2[1], self.additive)

        # concat skip connection and up-sampled layer from below
        skip_conn = (xd_2_b1 - xd_2_b2)
        attend_3 = gate_3(skip_conn, (xd_5_b1 - xd_5_b2))

        xu_3 = torch.cat((attend_3, pad_3(xu_3)), dim=1)

        xu_4_b = self.up_feat_ext_block_4(xu_3)
        xu_4 = self.up_block_4(xu_4_b)
        pad_4 = ReplicationPad2d((0, (s_1[3] - xu_4.shape[3]), 0, (s_1[2] - xu_4.shape[2])))

        if self.type_ == 'gate1':
            if self.sp:
                gate_4 = DualAttentionV3(256, s_1[1], self.additive, (8, 8))
            else:
                gate_4 = DualAttentionV3(256, s_1[1], self.additive)
        else:
            if self.sp:
                gate_4 = DualAttentionV4(256, s_1[1], self.additive, (8, 8))
            else:
                gate_4 = DualAttentionV4(256, s_1[1], self.additive)

        # concat skip connection and up-sampled layer from below
        skip_conn = (xd_1_b1 - xd_1_b2)
        attend_4 = gate_4(skip_conn, (xd_5_b1 - xd_5_b2))

        xu_4 = torch.cat((attend_4, pad_4(xu_4)), dim=1)

        xu_5_b = self.up_feat_ext_block_5(xu_4)
        xu_5 = self.up_block_5(xu_5_b)
        pad_5 = ReplicationPad2d((0, (s_0[3] - xu_5.shape[3]), 0, (s_0[2] - xu_5.shape[2])))

        x_out = self.out_block(pad_5(xu_5))

        return x_out


if __name__ == '__main__':
    t_1 = torch.randn(16, 3, 128, 128).cuda()
    t_2 = torch.randn(16, 3, 128, 128).cuda()
    model = FCFEResSplitAttention(3, additive=True, type_='gate2', sp=True)
    model.cuda()

    print(model(t_1, t_2).shape)

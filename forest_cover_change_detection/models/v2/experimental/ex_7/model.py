...
from forest_cover_change_detection.models.fcef.modules import UpSample
from forest_cover_change_detection.models.v2.experimental.ex_7.modules import StageZero, StageOne, StageTwo, StageThree


class AttentionEx1(nn.Module):

    def __init__(self, in_channels, classes, kernel=3):
        super(AttentionEx1, self).__init__()
        filters = [16, 32, 64, 128, 256]
        self.drop = nn.Dropout(0.2)

        # down sampling
        self.feat_ext_block_1 = StageZero(in_channels, filters[0])
        self.dwn_block_1 = nn.MaxPool2d(2)  # (16, 64, 64)

        self.feat_ext_block_2 = StageOne(filters[0], filters[1])
        self.dwn_block_2 = nn.MaxPool2d(2)  # (32, 32, 32)

        self.feat_ext_block_3 = StageTwo(filters[1], filters[2])
        self.dwn_block_3 = nn.MaxPool2d(2)  # (64, 16, 16)

        self.feat_ext_block_4 = StageThree(filters[2], filters[3])
        self.dwn_block_4 = nn.MaxPool2d(2)  # (128, 8, 8)

        self.feat_ext_block_5 = StageThree(filters[3], filters[4])
        self.dwn_block_5 = nn.MaxPool2d(2)  # (256, 4, 4)

        # up sampling
        self.up_feat_ext_block_1 = StageThree(filters[4], filters[4])
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_1 = UpSample(filters[4], filters[4], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (256, 8, 8)

        # this is a common up-sample block for all models
        self.up_feat_ext_block_2 = StageThree(3 * filters[3], filters[4])
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_2 = UpSample(filters[4], filters[3], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (128, 16, 16)

        # this is a common up-sample block for all models
        self.up_feat_ext_block_3 = StageTwo(3 * filters[2], filters[3])
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_3 = UpSample(filters[3], filters[2], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (64, 32, 32)

        # this is a common up-sample block for all models
        self.up_feat_ext_block_4 = StageOne(3 * filters[1], filters[2])
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_4 = UpSample(filters[2], filters[1], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (32, 64, 64)

        # this is a common up-sample block for all models
        self.up_feat_ext_block_5 = StageZero(3 * filters[0], filters[1])
        # this block is the layer that increases the dimensions by factor 2
        self.up_block_5 = UpSample(filters[1], filters[0], kernel,
                                   stride=2, padding=0, output_padding=1, blocks=1)  # (16, 128, 128)

        self.out_block = nn.Sequential(nn.Conv2d(filters[0], classes, kernel, padding=1),
                                       nn.LogSoftmax(dim=1))  # (2, 128, 128)

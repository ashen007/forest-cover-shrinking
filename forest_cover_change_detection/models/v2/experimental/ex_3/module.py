...
from forest_cover_change_detection.models.v2.experimental.new_gates.modules import DualAttentionV5


class FCFEResSplitAttention(nn.Module):

    def __init__(self, in_channels, classes, kernel=3, concat_ops='skip'):
    ...
    def forward(self, x_1, x_2):
        ...
        gate_1 = DualAttentionV5(xu_1_b.shape[1], s_4[1], (6, 2), self.concat_ops)

        # concat skip connection and up-sampled layer from below
        skip_conn = xd_4_b1 - xd_4_b2
        xu_1 = torch.cat((gate_1(skip_conn, xu_1_b), pad_1(xu_1)), dim=1)
        ...
        gate_2 = DualAttentionV5(xu_2_b.shape[1], s_3[1], (10, 6), self.concat_ops)

        # concat skip connection and up-sampled layer from below
        skip_conn = xd_3_b1 - xd_3_b2
        xu_2 = torch.cat((gate_2(skip_conn, xu_2_b), pad_2(xu_2)), dim=1)
        ...
        gate_3 = DualAttentionV5(xu_3_b.shape[1], s_2[1], (20, 12), self.concat_ops)

        # concat skip connection and up-sampled layer from below
        skip_conn = xd_2_b1 - xd_2_b2
        xu_3 = torch.cat((gate_3(skip_conn, xu_3_b), pad_3(xu_3)), dim=1)
        ...
        gate_4 = DualAttentionV5(xu_4_b.shape[1], s_1[1], (40, 24), self.concat_ops)

        # concat skip connection and up-sampled layer from below
        skip_conn = xd_1_b1 - xd_1_b2
        xu_4 = torch.cat((gate_4(skip_conn, xu_4_b), pad_4(xu_4)), dim=1)

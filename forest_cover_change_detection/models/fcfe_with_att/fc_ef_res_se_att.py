...
from forest_cover_change_detection.models.fcef.modules import ResidualSEDownSample, ResidualSEUpSample
from forest_cover_change_detection.models.fcfe_with_att.modules import AdditiveAttentionGate


class FCFEResSEAAT(nn.Module):

    def __init__(self, in_channels, classes, kernel=3):
        super(FCFEResSEAAT, self).__init__()
        filters = [16, 32, 64, 128, 256]
        self.drop = nn.Dropout(0.2)

        # down sampling
        self.dwn_block_1 = nn.Sequential(ResidualSEDownSample(in_channels, filters[0]),
                                         ResidualSEDownSample(filters[0], filters[0]),
                                         nn.MaxPool2d(2)  # (16, 128, 128)
                                         )
        self.dwn_block_2 = nn.Sequential(ResidualSEDownSample(filters[0], filters[1]),
                                         ResidualSEDownSample(filters[1], filters[1]),
                                         nn.MaxPool2d(2)  # (32, 64, 64)
                                         )
        self.dwn_block_3 = nn.Sequential(ResidualSEDownSample(filters[1], filters[2]),
                                         ResidualSEDownSample(filters[2], filters[2]),
                                         nn.MaxPool2d(2)  # (64, 32, 32)
                                         )
        self.dwn_block_4 = nn.Sequential(ResidualSEDownSample(filters[2], filters[3]),
                                         ResidualSEDownSample(filters[3], filters[3]),
                                         nn.MaxPool2d(2)  # (128, 16, 16)
                                         )
        self.dwn_block_5 = nn.Sequential(ResidualSEDownSample(filters[3], filters[4]),
                                         ResidualSEDownSample(filters[4], filters[4]),
                                         nn.MaxPool2d(2)  # (256, 8, 8)
                                         )

        # up sampling
        self.con_block_1 = nn.Sequential(ResidualSEUpSample(filters[4], filters[4]), )
        self.up_block_1 = nn.Sequential(nn.ConvTranspose2d(filters[4], filters[4], kernel,
                                                           stride=2, padding=0, output_padding=1))  # (256, 16, 16)

        self.con_block_2 = nn.Sequential(ResidualSEUpSample(3 * filters[3], filters[4]),
                                         ResidualSEUpSample(filters[4], filters[3]), )
        self.up_block_2 = nn.Sequential(nn.ConvTranspose2d(filters[3], filters[3], kernel,
                                                           stride=2, padding=0, output_padding=1))  # (128, 32, 32)

        self.con_block_3 = nn.Sequential(ResidualSEUpSample(3 * filters[2], filters[3]),
                                         ResidualSEUpSample(filters[3], filters[2]), )
        self.up_block_3 = nn.Sequential(nn.ConvTranspose2d(filters[2], filters[2], kernel,
                                                           stride=2, padding=0, output_padding=1))  # (64, 64, 64)

        self.con_block_4 = nn.Sequential(ResidualSEUpSample(3 * filters[1], filters[2]),
                                         ResidualSEUpSample(filters[2], filters[1]), )
        self.up_block_4 = nn.Sequential(nn.ConvTranspose2d(filters[1], filters[1], kernel,
                                                           stride=2, padding=0, output_padding=1))  # (32, 128, 128)

        self.con_block_5 = nn.Sequential(ResidualSEUpSample(3 * filters[0], filters[1]),
                                         ResidualSEUpSample(filters[1], filters[0]), )
        self.up_block_5 = nn.Sequential(nn.ConvTranspose2d(filters[0], filters[0], kernel,
                                                           stride=2, padding=0, output_padding=1))  # (16, 256, 256)

        self.out_block = nn.Sequential(ResidualSEUpSample(filters[0], filters[0]),
...
if __name__ == '__main__':
    t = torch.randn(16, 6, 256, 256).cuda()
    model = FCFEResSEAAT(6, 2)
    model.cuda()

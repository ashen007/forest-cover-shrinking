...
class FCFEResSE(nn.Module):
    ...
    def __init__(self, in_channels, classes, kernel=3):
        ...
        # up sampling
        self.up_block_1 = nn.Sequential(ResidualSEUpSample(filters[4], filters[4]),
                                        nn.ConvTranspose2d(filters[4], filters[4], kernel,
                                                           stride=2, padding=0, output_padding=1)
                                        )  # (256, 16, 16)
        self.up_block_2 = nn.Sequential(ResidualSEUpSample(3 * filters[3], filters[4]),
                                        ResidualSEUpSample(filters[4], filters[3]),
                                        nn.ConvTranspose2d(filters[3], filters[3], kernel,
                                                           stride=2, padding=0, output_padding=1)
                                        )  # (128, 32, 32)
        self.up_block_3 = nn.Sequential(ResidualSEUpSample(3 * filters[2], filters[3]),
                                        ResidualSEUpSample(filters[3], filters[2]),
                                        nn.ConvTranspose2d(filters[2], filters[2], kernel,
                                                           stride=2, padding=0, output_padding=1)
                                        )  # (64, 64, 64)
        self.up_block_4 = nn.Sequential(ResidualSEUpSample(3 * filters[1], filters[2]),
                                        ResidualSEUpSample(filters[2], filters[1]),
                                        nn.ConvTranspose2d(filters[1], filters[1], kernel,
                                                           stride=2, padding=0, output_padding=1)
                                        )  # (32, 128, 128)
        self.up_block_5 = nn.Sequential(ResidualSEUpSample(3 * filters[0], filters[1]),
